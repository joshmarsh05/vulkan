...
bool VulkanRenderer::OnCreate(){ 
    ...
    // colour picker
    colorPickBindImageInfo();
    //the image view
    createOffScreenImageView();
    createColorPickerRenderPass();
    createColorPickerFrameBuffer();
    createStagingBuffer(stagingBuffer);
    createColorPickerCommandPool();
    createColorPickerCommandBuffer();
...
// change this to have multiple descriptorsets
// change the 1 to 2 when i have multiple sets one for lights and camera, another for texture(actor)
void VulkanRenderer::BindDescriptorSet(VkPipelineLayout pipelineLayout, const std::vector<VkDescriptorSet>& descriptorSet){
    for (uint32_t i = 0; i < numSwapchains; i++) {
        vkCmdBindDescriptorSets(primaryCommandBuffer.commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
            pipelineLayout, 0, descriptorSet.size(), descriptorSet.data(), 0, nullptr);
    }
}
