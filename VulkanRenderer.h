...
public: /// Member functions
    ...
    PipelineInfo CreateGraphicsPipeline(int setLayoutCount, VkDescriptorSetLayout* descriptorSetLayout, const char* vertFile, const char* fragFile,
...
public:
    ...
    void UpdateUniformBuffer(const T& srcData, const std::vector<BufferMemory> bufferMemory) {
        ...
        for (int i = 0; i < numSwapchains; ++i) {
            // device == gpu; &data == pointer to a pointer
...
private:
    ...
    ///////////////////////////////// Colour Picker /////////////////////////////////////
private:
    ColorPickerCommandBufferData colorPickerCommandBufferData;
    // color picker
    VkImageView colorPickerImageView;
    VkImage colorPickerImage;
    VkDeviceMemory pickImageMem;
    VkFramebuffer colorPickerFrameBuffer;
    VkRenderPass colorPickerRenderPass;
    VkFormat colorPickerImageFormat = VK_FORMAT_R8G8B8A8_UNORM;
...
public:
    ...
    void colorPickBindImageInfo();
    void createOffScreenImageView();
    void createColorPickerRenderPass();
    void createColorPickerFrameBuffer();
    void createColorPickerCommandPool();
    void RecordCPCommandBuffer(Recording Start_stop);
    void DestroyCPCommandBuffers();
    void BindColorPickerPipeline(VkPipeline pipeline);
    void BindCPDescriptorSet(VkPipelineLayout pipelineLayout, VkDescriptorSet descriptorSet, uint32_t descriptorIndex);
    void SetPushConstantCP(const ColorPickerPipelineInfo pipelineInfo, Actor* actor);
    void BindMeshCP(IndexedVertexBuffer mesh_);
    void DrawIndexedCP(IndexedVertexBuffer mesh_);
    void createStagingBuffer(BufferMemory& buffer);
    void createColorPickerCommandBuffer();
    void copyImagetoBuffer();
    void destroyColorPickerInfo();
    void DestroyCPPipeline(ColorPickerPipelineInfo pipelineInfo);
    void DestroyCPDescriptorSet(DescriptorSetInfo& descriptorSetInfo);
     /* VkCommandBuffer getCPCommandBuffer() {
        return colorPickerCommandBufferData.colorPickerCommandBuffer;
        }*/

    uint32_t extractColorID(int x_, int y_);
    ColorPickerPipelineInfo createColorPickerPipeline(VkDescriptorSetLayout descriptorSetLayout, const char* vertFile, const char* fragFile, 
        const char* tessCtrlFile = nullptr, const char* tessEvalFile = nullptr, const char* geomFile = nullptr, const char* colorPickingFile = nullptr);
