...
IndexedVertexBuffer VulkanRenderer::LoadModelIndexed(const char* filename) {
    ...
    // using a unordered map to store all the unique vertexs to remove any duplicates
    ...
    for (const auto& shape : shapes) {
        for (const auto& index : shape.mesh.indices) {
            ...
            // hashes the vertex if the vertex already exist nothing but if the vertex is new, it adds it
            // using the .count() to check if the vertex already exist
